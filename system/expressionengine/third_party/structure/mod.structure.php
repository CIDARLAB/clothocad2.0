	...
	function nav_main()
	{
		...
		if (count($result->num_rows()) > 0)
		{
			...
			foreach ($result->result_array() as $entry_data)
			{
				...
				$item_uri = ($title == 'Wiki')?'http://clothohelp.org':$item_uri;

				// Add a <span> wrapper inside the <a> tags
				if ($add_span == 'yes')
				{
					$html .= "\n<li{$li_class}{$list_item_id}><a href=\"" . $item_uri . "\"{$a_class}".(($title == 'Wiki')?' target="_blank"':'')."><span>" . $title."</span></a></li>";
				}
				else
				{
					$html .= "\n<li{$li_class}{$list_item_id}><a href=\"" . $item_uri . "\"{$a_class}".(($title == 'Wiki')?' target="_blank"':'').">" . $title."</a></li>";
				}
			}
		...
		function nav_sub()
		{
			...
			if ($fixed && $current_node['parent_id'] != 0)
			{
				...
				foreach ($pages as $key => $page)
				{
					if (($page['depth'] < 0) OR ( ! in_array($page['parent_id'], $tree) && !in_array($page['entry_id'], $tree) && $page['parent_id'] != $node['id']))
					{
						unset($pages[$key]);
					}
				}
